...
public static class ManifestPipeline
{
    ...
    public static PluginManifest Build(
        ...
        bool noBuild = false)
    {
        Log.Heading($"Discovering plugin assemblies in solution '{paths.Name}'");
    ...
    private static string Relative(SolutionPaths paths, string path) =>
        Path.GetRelativePath(paths.Repo.Root, path).Replace('\\', '/');
}
