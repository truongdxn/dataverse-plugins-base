...
public static class CommandHandlers
{
    ...
    public static int Solutions(CommandLine cli)
    {
        ...
        if (set.All.Count == 0)
        {
            Log.Warn(
                ...
                "Create one with 'dotnet new dv-solution -n <Name>'.");
            return 0;
        }
            ...
            throw new ToolException(
                string.IsNullOrWhiteSpace(assemblyName)
                    ? $"Solution '{paths.Name}' has no test projects. A test project is one that " +
                      "imports Testing.Sources.props. Add one with 'dotnet new dv-plugin-tests -n <Name>'."
    ...
    public static int Validate(CommandLine cli)
    {
        ...
        if (missing.Count > 0)
        {
            Log.Warn(
                ...
                $"{Path.GetFileName(paths.SdkMessageCacheFile)}.");
        }
    ...
    public static int Pack(CommandLine cli)
    {
        ...
        if (missing.Count > 0)
        {
            throw new ToolException(
                ...
                "Run 'dv messages pull --env <name>' once, then commit " +
                $"{Path.GetFileName(paths.SdkMessageCacheFile)}.");
        }
    ...
    /// <summary>Populates the solution's sdkmessages.json so packing can run offline afterwards.</summary>
    ...
    {
        var (paths, _) = Resolve(cli, loadConfig: false);
        var environmentName = cli.RequiredOption("env");
        var environment = EnvironmentConfig.Load(paths.Repo).Get(environmentName);
    ...
    private static int WriteGeneratedSchema(SolutionPaths paths, Model.SchemaSnapshot snapshot)
    {
