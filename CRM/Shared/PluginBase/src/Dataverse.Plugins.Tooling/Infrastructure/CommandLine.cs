...
public sealed class CommandLine
{
    ...
    /// <summary>
    /// Never offered as a "did you mean". --prune deletes registrations, so it must be typed
    /// deliberately - proposing it to somebody who has already mistyped something is the last
    /// thing a tool should do. It still works when spelled out in full.
    /// </summary>
    private static readonly HashSet<string> NeverSuggested = new(StringComparer.OrdinalIgnoreCase)
    {
        "prune",
    };
    ...
    private static IEnumerable<string> Suggestable =>
        KnownNames.Where(name => !NeverSuggested.Contains(name));
    ...
    private static string ClosestMatch(string name)
    {
        ...
        foreach (var known in Suggestable)
        {
