...
public class CommandLineTests
{
    ...
    /// harmless one, so it deliberately has no short form. Only the spelled-out name reaches it.
    /// </summary>
    [Fact]
    public void PruneHasNoShortForm()
    {
        Assert.Throws<ToolException>(() => CommandLine.Parse(["sync", "-p"]));

        Assert.True(CommandLine.Parse(["sync", "--prune"]).HasFlag("prune"));
    }

    /// <summary>
    /// Suggesting a destructive flag to somebody who has already mistyped something is the last
    /// thing this tool should do. '-p' is close enough to '--prune' that it otherwise would.
    /// </summary>
    [Theory]
    [InlineData("-p")]
    [InlineData("--pru")]
    [InlineData("--prun")]
    public void ATypoIsNeverAnsweredWithASuggestionOfPrune(string typo)
    {
        var exception = Assert.Throws<ToolException>(() => CommandLine.Parse(["sync", typo]));

        Assert.DoesNotContain("prune", exception.Message, StringComparison.OrdinalIgnoreCase);
    }
